...
content = re.sub(r"'無修復期'", '"無修復期"', content)
content = re.sub(r"'享9折優惠", '"享9折優惠"', content)
content = re.sub(r"'超過三個月，依照價目表收費'", '"超過三個月，依照價目表收費"', content)

# 修復3: 修復 renderEyebrowDesign 函數的標題結構 (428-441行區域)
...
content = re.sub(
    r'<h2 class="mb-8 text-center"[^>]*>\s*<div class="card[^>]*>.*?眉型設計展示\s*</h2>',
    new_eyebrow,
    content,
    flags=re.DOTALL
)
